...
if [ -f /app/keep_alive.py ]; then
  echo ">> Starting keep-aliv#!/bin/bash
export PATH="$HOME/.local/bin:/usr/local/bin:$PATH"
export TZ="Asia/Kolkata"
...
# Fix: आधीचा regex फक्त "model:" line replace करायचा, पण "default: anthropic/..."
# सारख्या indented sub-keys तशाच राहायच्या → YAML syntax error line 60.
# आता पूर्ण model: block (सगळ्या indented lines सकट) एकाच scalar ने replace करतो.
python3 -c "
import os, re
...
try:
    ...
    # model patch — पूर्ण block replace (scalar किंवा nested mapping दोन्ही handle करतो)
    # ^model: नंतर same line + सगळ्या indented continuation lines एकत्र match
